...
\FOR{$i = 0$ \TO $M$}
    \STATE $S_i \leftarrow i \cdot \Delta S$
    \STATE $V[i] \leftarrow \max(K - S_i, 0)$ \COMMENT{Put option payoff}
\ENDFOR
...
\STATE $V[0] \leftarrow K \cdot e^{-rT}$ \COMMENT{At $S = 0$, option worth discounted strike}
\STATE $V[M] \leftarrow 0$ \COMMENT{At $S = S_{max}$, put option worthless}
...
\STATE $\tau \leftarrow t$ \COMMENT{Time to expiration from current time}
\STATE $V[0] \leftarrow K \cdot e^{-r\tau}$ \COMMENT{Lower boundary: discounted strike}
\STATE $V[M] \leftarrow 0$ \COMMENT{Upper boundary: option worthless}

\end{algorithmic}
\end{algorithm}

\begin{algorithm}[H]
\caption{Interpolate-Value-Implicit}
\begin{algorithmic}[1]
\REQUIRE Grid $V$, $S_0$ (target stock price), $\Delta S$ (grid spacing)
\ENSURE Interpolated option value at $S_0$

\STATE $i \leftarrow \lfloor S_0 / \Delta S \rfloor$ \COMMENT{Find lower grid index}
\STATE $\lambda \leftarrow (S_0 - i \cdot \Delta S) / \Delta S$ \COMMENT{Interpolation weight}

\IF{$i \geq$ size$(V) - 1$}
    \RETURN $V[\text{size}(V) - 1]$ \COMMENT{Boundary case}
\ENDIF

\STATE $\text{value} \leftarrow (1 - \lambda) \cdot V[i] + \lambda \cdot V[i+1]$ \COMMENT{Linear interpolation}
